...
def get_chunks(docs: List, chunk_size: int = 3000, chunk_overlap: int = 500) -> List:
    """
...
def store_chunks_into_vectorstore(
    vector_db: Chroma, chunks: List, api_key: str
    ...
    embeddings = OpenAIEmbeddings(api_key=api_key)
    # Create vector store
    vectorstore = vector_db.from_documents(documents=chunks, embedding=embeddings)
...
def run_preprocess(pdf_path: str, api_key: str) -> ContextualCompressionRetriever:
    ...
    retriever = store_chunks_into_vectorstore(vector_db, chunks, api_key)
...
def get_vectorstore_compressed_retriever(
    vector_db, chunks, api_key: str
) -> ContextualCompressionRetriever:
...
# run_preprocess(
#     "/Users/user/Documents/agent_rag_langgraph/agent_rag_langgraph/data/gels-pdt-gpa-brochure.pdf", api_key
# )
# chunks = preprocess_document_to_markdown(
#     "/Users/user/Documents/agent_rag_langgraph/agent_rag_langgraph/data/gels-pdt-gpa-brochure.pdf"
# )
# store = create_vector_store(
#     api_key
# )
# compression_retriever = get_vectorstore_compressed_retriever(
#     store,
#     chunks,
#     api_key,
# )
